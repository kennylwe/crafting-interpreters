...
(* (1* if 1 + 1 < 2 then 10 + 1 else 2 * 3 *1) *)
...
(*   (1* let simpl_int e = get_int (simplify e) in *1) *)
(*   match y with *)
(*   | Int x -> Int x *)
(*   | Bool x -> Bool x *)
(*   | Add (x, y) -> *)
...
(*     (1* [if] is a special form, "normal", doesn't compute unless n *1) *)
(*     (1* [+] is not a special form, "applicative", immediately applies *1) *)
